...
function prettyPrintErrorLine(line, errorIdx, lineNum) {
	...
	return `Line ${ lineNum }, Col ${ errorIdx + 1 }\n${ line }\n${ pointer }`
}
...
function parseLine(rawLine, lineIndex) {
	...
	while (idx < line.length && error == null) {
		...
		if (beforeEquals && isWhitespace(char)) {
			let seek = idx
			let current = ' '

			while (seek < line.length && isWhitespace(current)) {
				current = line.charAt(seek)
				if (current === '=') {
					idx = seek
					...
					error = `Found spaces within variable name @ ${ prettyPrintErrorLine(line, seek, lineNum) }`
					break processline
				}
			}
		}
		...
		if (isWhitespace(char)) {
			if (beforeEquals) {
				...
				break
			} else if (idx !== 0 && line.charAt(idx - 1) === '=') {
				skip = true
			} else if (!quoted) {
